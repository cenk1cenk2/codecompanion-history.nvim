...
T["Adapter Extraction"] = new_set()

T["Adapter Extraction"]["saves adapter from chat.adapter.name for HTTP adapters"] = function()
    local result = child.lua([[
        test_storage:save_chat({
            opts = {
                save_id = "test_http_adapter",
                title = "HTTP Adapter Chat",
            },
            messages = {
                { role = "user", content = "hello" },
            },
        ...
        local loaded = test_storage:load_chat("test_http_adapter")
        local index = test_storage:get_chats()

        return {
            chat_adapter = loaded.adapter,
            index_adapter = index["test_http_adapter"].adapter,
        }
    ]])

    eq("openai", result.chat_adapter)
    eq("openai", result.index_adapter)
end

T["Adapter Extraction"]["prefers ACP connection adapter over chat.adapter"] = function()
    local result = child.lua([[
        -- Simulate a chat where chat.adapter still shows the old adapter
        -- but the ACP connection has the correct (switched) adapter
        test_storage:save_chat({
            opts = {
                save_id = "test_acp_adapter_switch",
                title = "ACP Adapter Switch",
            },
            messages = {
                { role = "user", content = "hello" },
            },
            settings = {},
            adapter = { name = "claude_code" },  -- old adapter (stale)
            acp_connection = {
                adapter = { name = "codex" },  -- actual adapter after switch
                _models = { currentModelId = "o3-pro" },
            },
            cycle = 1,
        })

        local loaded = test_storage:load_chat("test_acp_adapter_switch")
        local index = test_storage:get_chats()

        return {
            chat_adapter = loaded.adapter,
            index_adapter = index["test_acp_adapter_switch"].adapter,
        }
    ]])

    eq("codex", result.chat_adapter)
    eq("codex", result.index_adapter)
end

T["Adapter Extraction"]["uses chat.adapter.name when no ACP connection"] = function()
    local result = child.lua([[
        test_storage:save_chat({
            opts = {
                save_id = "test_no_connection",
        ...
        local loaded = test_storage:load_chat("test_no_connection")

        return { chat_adapter = loaded.adapter }
    ]])

    eq("claude_code", result.chat_adapter)
end

T["Adapter Extraction"]["returns unknown when no adapter info available"] = function()
    local result = child.lua([[
        test_storage:save_chat({
            opts = {
                save_id = "test_no_adapter",
                title = "No Adapter Chat",
            },
            messages = {
                { role = "user", content = "hello" },
            },
        ...
        local loaded = test_storage:load_chat("test_no_adapter")

        return { chat_adapter = loaded.adapter }
    ]])

    eq("unknown", result.chat_adapter)
end
